...
const fetchNearbyPlacesGeoapify = async (lat, lon) => {
  try {
    ...
    // Map the data and then slice to only keep the first 5
    const places = data.features
      .map((p) => ({
        ...
        category: p.properties.categories?.[0]?.split('.').pop() || "Tourist Spot",
      }))
      .slice(0, 5); // 🔹 This ensures only 5 items are stored
